...
<motion.div layout className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-10">
  ...
  key={post.id}
  layout
  initial={{ opacity: 0, scale: 0.8 }} // 初始縮小更多 (0.8)
  animate={{ opacity: 1, scale: 1 }}
  exit={{ opacity: 0, scale: 0.8 }}
  transition={{
            opacity: { duration: 0.4 }, // opacity 用普通的
            layout: { type: "spring", stiffness: 300, damping: 30 }, // 彈簧效果
            scale: { type: "spring", stiffness: 300, damping: 30 }   // scale 也用彈簧
          }}
>
          <AnimatedLink
